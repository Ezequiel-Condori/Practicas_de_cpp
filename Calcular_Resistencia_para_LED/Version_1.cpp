...
void ResistLED(float Valor_1, float Valor_2){
    ...
    coat<<"La Resistencia que deberia usar es de valor: "<<VoltFinal/IntensLED<<"Ohm's\n\n";
}
...
int main(){
    ...
    do{
        ...
        if(Opcion=="NARANJA"||Opcion=="AMARILLO"){
            VoltLED=2.1;
            ResistLED(VoltFuente, VoltLED);
        }
        if(Opcion=="VERDE_ESTANDAR"){
            VoltLED=2.2;
            ResistLED(VoltFuente, VoltLED);
        }
        if(Opcion=="VERDE_ESMERALDA"){
            VoltLED=3.2;
            ResistLED(VoltFuente, VoltLED);
        }
        if(Opcion=="AZUL"){
